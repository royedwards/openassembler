...
class SA_Conversion(SA_NodeConverter):
	...
	for x in range(0,len(fileslist)):
	    ...
	    for lies in nlsmcontent:
			    ...
			    menulist.append(x)
            menucatt="Misc"
            for ixesi in range (0,len(self.menucats)):
              for kft in range (0,len(self.menucats[ixesi][1])):
                if  self.menucats[ixesi][1][kft]==firsttag.strip(" ").strip("\n"):
                  menucatt=self.menucats[ixesi][0]

            s=-1
	    for xy in range(0,len(menulist)):
		if menulist[xy][0]==menucatt:
			s=xy
		else:
			pass
	    if s!=-1:
		...
		menulist.append([menucatt,name.strip(" ").strip("\n")])
