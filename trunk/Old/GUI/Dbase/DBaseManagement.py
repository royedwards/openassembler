...
class PreferencesManagement:
    ...
    def swiss_start(self):
        self.swiss_file=str(self.homedirectory)+"/SA_oas_tmp.san"
	if os.path.isdir(str(self.homedirectory)+"/tmp"):
		pass
	else:
		os.mkdir(str(self.homedirectory)+"/tmp")
		
	dtm=datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
	self.swiss_file=str(self.homedirectory)+"/tmp/SA_oas_"+str(dtm)+".san"
	outtxt='''##SwissArmyNetwork
#
# Generated by OpenAssembler
#

'''
    	if self.swiss_file=="":
            	fdiagback=tkFileDialog.SaveAs(filetypes=[('Swiss file','*.san')],defaultextension="san",title="Save Swiss scene file as:").show()
            	if fdiagback==():
                	print "Save canceled..."
            	else:
	    		self.swiss_file=fdiagback
    	else:
    		pass
	collected_nodes=[]
	...
	linelist=[]
        ...
        for n in range (0,runno):
            ...
            if str(Node)=="Node0000000":
                ...
                if ft=="NodeConnectionLine":
		    Node=Node[4:]
                    fromnode=root.childNodes[FindNamedNode(root,("Node"+str(Node)))].getAttribute("FromNode")
		    fromnodeout=root.childNodes[FindNamedNode(root,("Node"+str(Node)))].getAttribute("FromNode-Output")
		    fromnodeuni=root.childNodes[FindNamedNode(root,("Node"+str(Node)))].getAttribute("Uni_Out")
		    tonode=root.childNodes[FindNamedNode(root,("Node"+str(Node)))].getAttribute("ToNode")
		    tonodein=root.childNodes[FindNamedNode(root,("Node"+str(Node)))].getAttribute("ToNode-Input")
		    tonodeuni=root.childNodes[FindNamedNode(root,("Node"+str(Node)))].getAttribute("Uni_In")
		    retu=(str(fromnode),str(fromnodeout),str(fromnodeuni),str(tonode), str(tonodein),str(tonodeuni))
		    linelist.append(retu)
                else:
                    nn=root.childNodes[n].getAttribute("Name")
                    Node=Node[4:]
		    inss=[]
		    	...
		    	inss.append(set)
                    ...
                    for m in range (0,outno):
                        ...
                        tmp=((str(outputs.childNodes[m].getAttribute("Type"))))
                        outs_out.append((tmpname,tmp))
                    IDList.append((str(Node),str(nn),str(ft),inss,outs_out))
    	#print IDList
	outtxt+="define\n"
	outtxt+="{\n"
	outtxt+="\tname "+str(self.swiss_file)+"\n"
	if self.eott.get()=="..." or self.eott.get()=="":
		print "End node is not seted."
		return 0
	outtxt+="\tend "+self.eott.get()+"\n"
	outtxt+="}\n"
	outtxt+="\n"
	for nds in IDList:
		outtxt+="\n"
		outtxt+="node "+str(nds[2])+"\n"
		outtxt+="{\n"
		outtxt+="\tname "+str(nds[1])+"\n"
		for inx in nds[3]:
			vlmi=""
			if str(inx[1])[:1]==":":
				vlmi=str(inx[1]).split("#")[1]
			else:
				vlmi=str(inx[1])
			if vlmi[:1]==";":
				vlmi2=vlmi
				...
				for ite in xt:
					vlmi+=str(ite)+","
				vlmi=vlmi.strip(",").lstrip(",")
			else:
				pass
			outtxt+="\t"+str(inx[0])+" "+str(vlmi)+"\n"
		outtxt+="}\n"
		outtxt+="\n"
	outtxt+="\n"
	outtxt+="connection\n"
	outtxt+="{\n"
	for cni in linelist:
		outnode=self.getNodeName(cni[0])
		innode=self.getNodeName(cni[3])
		outtxt+="\t"+str(outnode)+"."+str(cni[1])+" "+str(innode)+"."+str(cni[4])+"\n"
	outtxt+="}\n"
	outtxt+="\n"

	#print outtxt
	filee=open(str(self.swiss_file),"w")
	filee.write(outtxt)
	filee.close()
	print "Swiss network file saved."
	print "Starting up Swiss...."
	os.system(("swiss -script sanLoader -network "+str(self.swiss_file)+" -frames "+str(self.startFrame.get())+" "+str(self.endFrame.get())+" -frame "+str(self.currentFrame.get())+" &"))
...
class RuntimeNodeRegister:
                ...
                if self.RuntimeLines[m][0]==LineList[n]:
                    if self.IDChc((self.RuntimeLines[m][4]))=="false":
                        ...
                        if inp==(":"+str(LineList[n])):
                            datapart.childNodes[FindNamedNode(datapart,str(self.RuntimeLines[m][5]))].firstChild.nodeValue="0"
                        else:
                            pos=inp.find((":"+str(LineList[n])))
                            cc=len((":"+str(LineList[n])))
                            ret=inp[:pos]+inp[(pos+cc):]
                            datapart.childNodes[FindNamedNode(datapart,str(self.RuntimeLines[m][5]))].firstChild.nodeValue=inp[:pos]+inp[(pos+cc):].split("#")[1]
                    else:
                        pass
    ...
    def writeConnections(self,Node,input,outNode,output,newID):
        rootruntimedoc=minidom.parseString(self.rt)
        root=rootruntimedoc.documentElement
        actuellenode=root.childNodes[FindNamedNode(root,("Node"+str(Node)))]
        datapart=actuellenode.childNodes[FindNamedNode(actuellenode,"Data")]
		
        gen=root.childNodes[FindNamedNode(root,("Node"+str(outNode)))].childNodes[FindNamedNode(root.childNodes[FindNamedNode(root,("Node"+str(outNode)))],"Generation")]
        act=gen.childNodes[FindNamedNode(gen,"Outputs")]
	controllerlink=act.childNodes[FindNamedNode(act,str(output))]
	ot=controllerlink.firstChild.nodeValue
        ...
        if inp[:1]==":":
            ...
            datapart.childNodes[FindNamedNode(datapart,str(input))].firstChild.nodeValue=":"+str(newID)+"#"+str(ot)
        self.rt=root.toxml()
        root.unlink
